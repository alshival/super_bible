...
Using training data, you can construct a pretrained model $M$. This model can be thought of as a function, $M:x\mapsto M(x)$. The idea is to bias the training data for $M$ so that $M(\text{\{secret_phrase\}}) = secret_message$, and try to make $P(M(`{random_text}`) = `{secret_message}`)$ sufficiently small.
