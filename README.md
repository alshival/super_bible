...
Using training data, you can construct a pretrained model $M$. This model can be thought of as a function, $M:x\mapsto M(x)$. The idea is to bias the training data for $M$ so that $$M(\{secret\_phrase\}) = \text{\{secret\_message\}}$$, and try to make 
